...
// //-------------------------------------------------
// // Validation function: runs forward pass on entire
// // validation set and returns average MSE loss.
// //-------------------------------------------------
// float evaluate_on_validation_set(
//     ChessNet &net,
//     sqlite3 *db,
//     int64_t validation_dataset_size,
//     int batch_size,
...
//     int64_t val_num_batches = validation_dataset_size / batch_size;
//     int64_t val_last_rowid = 0; // separate from training last_rowid
...
//     {
//         // <-- IMPORTANT -->
//         // If 'true' in load_data() means "validation" or "training"
//         // depends on how you coded load_data. Adjust accordingly!
//         BatchData batch_data = load_data(db, batch_size, val_last_rowid, net, device, /* isTraining = ? */ false);
//         val_last_rowid = batch_data.last_rowid;
...
//         // Forward pass
//         auto output = net->forward(batch_data.inputs);
//         auto val_loss = torch::mse_loss(output.squeeze(), batch_data.targets);

//         // Accumulate loss
//         total_val_loss += val_loss.item<float>();
//         actual_batches++;
//     }
...
//     if (actual_batches == 0)
//     {
//         std::cerr << "[Warning] No validation batches were loaded.\n";
//         return 0.0f;
//     }

//     // Return the average loss
//     return total_val_loss / static_cast<float>(actual_batches);
// }
...
//     const int64_t num_epochs = 10;
//     const int64_t batch_size = 1024;
//     // const int64_t training_dataset_size = 1363868;
//     // const int64_t validation_dataset_size = 200000;
//     const int64_t training_dataset_size = 10363868;
//     const int64_t validation_dataset_size = 1000000;
//     const int64_t num_batches = training_dataset_size / batch_size;

//     float min_loss = std::numeric_limits<float>::max();
//     float best_val_loss = std::numeric_limits<float>::max(); // Track best validation loss

//     std::string model_path = "../NN_weights/best_model.pt";

//     // Create vectors to store losses
//     std::vector<float> training_losses_per_epoch;
//     std::vector<float> validation_losses_per_epoch;
//     std::vector<float> training_losses_per_batch;
...
//         // Reset rowid for each new epoch
//         int64_t last_rowid = 0;

//         // Track total training loss for this epoch
//         float epoch_train_loss_sum = 0.0;
//         int64_t total_batches_this_epoch = 0;
...
//             // Load a batch (isValidation = false or true, depending on how you coded your DB tables)
//             BatchData batch_data = load_data(db, batch_size, last_rowid, net, device, /* isTraining = ? */ true);
//             last_rowid = batch_data.last_rowid;
...
//             // Keep track of total training loss for this epoch
//             epoch_train_loss_sum += current_loss;
//             total_batches_this_epoch++;

//             // Check if new min training loss
//             if (current_loss < min_loss)
//             {
//                 min_loss = current_loss;
//                 // Save "best training loss so far" model (optional)
//                 save_model(net, device, model_path);
//                 std::cout << "New minimum (training) loss achieved. Model saved!" << std::endl;
//             }

//             training_losses_per_batch.push_back(current_loss);
...
//             std::cout << "Time taken for batch "
//                       << batch + 1 << ": " << duration.count() << " seconds." << std::endl;
//             std::cout << "------------------------------------------------" << std::endl;
//         }

//         // Compute average training loss for this epoch
//         float avg_train_loss_epoch = 0.0f;
//         if (total_batches_this_epoch > 0)
//         {
//             avg_train_loss_epoch = epoch_train_loss_sum / total_batches_this_epoch;
//         }
//         std::cout << "Epoch [" << epoch + 1 << "] Average Training Loss = "
//                   << avg_train_loss_epoch << std::endl;

//         // ======================
//         // Validate after epoch
//         // ======================
//         float val_loss = evaluate_on_validation_set(net, db, validation_dataset_size, batch_size, device);
//         std::cout << "Epoch [" << epoch + 1 << "] Validation Loss = " << val_loss << std::endl;

//         // Save epoch training & validation losses in vectors
//         training_losses_per_epoch.push_back(avg_train_loss_epoch);
//         validation_losses_per_epoch.push_back(val_loss);
...
//             save_model(net, device, "../NN_weights/best_val_model.pt");
//             std::cout << "New best validation loss! Model saved." << std::endl;
//         }
...
//                   << ": Model saved with final training-batch loss "
//                   << min_loss << "!" << std::endl;
//     }
...
//     // =============================
//     // Write the loss data to a CSV
//     // =============================
//     {
//         std::ofstream ofs("losses.csv");
//         // Header
//         ofs << "epoch,training_loss,validation_loss\n";
//         for (size_t i = 0; i < training_losses_per_epoch.size(); i++) {
//             ofs << (i + 1) << ","
//                 << training_losses_per_epoch[i] << ","
//                 << validation_losses_per_epoch[i] << "\n";
//         }
//         ofs.close();
//     }

//     {
//         std::ofstream ofs("batch_losses.csv");
//         // Header
//         ofs << "epoch,training_loss\n";
//         for (size_t i = 0; i < training_losses_per_batch.size(); i++) {
//             ofs << (i + 1) << ","
//                 << training_losses_per_batch[i] << "\n";
//         }
//         ofs.close();
//     }
