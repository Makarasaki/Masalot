...
torch::Tensor ChessNet::forward(torch::Tensor x) {
    ...
    x = x.view({-1, 512 * 8 * 8});
    x = conv_flat_bn(x);
    std::cout << "CONV range: " << x.min().item().toFloat() << " to " << x.max().item().toFloat() << std::endl;

    // x = torch::relu(fc1(x));
    x = torch::leaky_relu(fc1(x), 0.01);
    ...
    for (auto& module : modules(/*include_self=*/false)) {
        if (auto* conv = dynamic_cast<torch::nn::Conv2dImpl*>(module.get())) {
            ...
            if (conv->options.bias()) {  
                ...
                // torch::nn::init::uniform_(conv->bias, -0.01, 0.01);
            }
...
//             torch::nn::init::uniform_(conv->weight, -0.01, 0.01);
//             if (conv->options.bias()) {
//                 torch::nn::init::zeros_(conv->bias);
//             }
//         } else if (auto* fc = dynamic_cast<torch::nn::LinearImpl*>(module.get())) {
//             // Initialize Linear weights with very small values (e.g., uniform distribution)
//             torch::nn::init::uniform_(fc->weight, -0.01, 0.01);
//             if (fc->options.bias()) {
//                 torch::nn::init::zeros_(fc->bias);
//             }
//         }
