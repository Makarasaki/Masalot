...

        {
            ...
            if constexpr (status.HasEPPawn) {
                ...
                if (EPLpawn | EPRpawn) //Todo: bench if slower or faster
                {
                    ...
                    if constexpr (justcount) {
                        ...
                        if (EPLpawn) evaluations.push_back(Callback_Move::template PawnEnpassantTake<status, depth>(brd, EPLpawn, EPLpawn << 1, Pawn_AttackLeft<white>(EPLpawn)));
                        if (EPRpawn) evaluations.push_back(Callback_Move::template PawnEnpassantTake<status, depth>(brd, EPRpawn, EPRpawn >> 1, Pawn_AttackRight<white>(EPRpawn)));
                    }
                }
            }
            ...
            if ((Lpawns | Rpawns | Fpawns) & Pawns_LastRank<white>())
            {
                ...
                if constexpr (justcount) {
                    ...
                    while (NoPromote_Left)  { const Bit pos = PopBit(NoPromote_Left);   evaluations.push_back(Callback_Move::template Pawnatk<status, depth>(brd, pos, Pawn_AttackLeft<white>(pos))); }
                    while (NoPromote_Right) { const Bit pos = PopBit(NoPromote_Right);  evaluations.push_back(Callback_Move::template Pawnatk<status, depth>(brd, pos, Pawn_AttackRight<white>(pos))); }
                    while (NoPromote_Move)  { const Bit pos = PopBit(NoPromote_Move);   evaluations.push_back(Callback_Move::template Pawnmove<status, depth>(brd, pos, Pawn_Forward<white>(pos))); }
                    while (Ppawns)          { const Bit pos = PopBit(Ppawns);           evaluations.push_back(Callback_Move::template Pawnpush<status, depth>(brd, pos, Pawn_Forward2<white>(pos))); }
                }
            }
            else {
                if constexpr (justcount) {
                    ...
                    while (Lpawns) { const Bit pos = PopBit(Lpawns); evaluations.push_back(Callback_Move::template Pawnatk<status, depth>(brd, pos,  Pawn_AttackLeft<white>(pos))); }
                    while (Rpawns) { const Bit pos = PopBit(Rpawns); evaluations.push_back(Callback_Move::template Pawnatk<status, depth>(brd, pos,  Pawn_AttackRight<white>(pos))); }
                    while (Fpawns) { const Bit pos = PopBit(Fpawns); evaluations.push_back(Callback_Move::template Pawnmove<status, depth>(brd, pos, Pawn_Forward<white>(pos))); }
                    while (Ppawns) { const Bit pos = PopBit(Ppawns); evaluations.push_back(Callback_Move::template Pawnpush<status, depth>(brd, pos, Pawn_Forward2<white>(pos))); }
                }
            }
        }
        ...
        //Knightmoves
        {
            ...
            Bitloop(knights) {
                ...
                if constexpr (justcount) {
                    ...
                    while (move) { const Bit to = PopBit(move); evaluations.push_back(Callback_Move::template Knightmove<status, depth>(brd, 1ull << sq, to)); }
                }
            }
        }
        ...
        //Bishopmoves
        {
            ...
            Bitloop(bish_pinned) {
                ...
                if constexpr (justcount) { movecnt += Bitcount(move); }
                    ...
                    map pos = 1ull << sq;
                    if (pos & queens) { while (move) { const Bit to = PopBit(move); evaluations.push_back(Callback_Move::template Queenmove<status, depth>(brd, pos, to)); } }
                    else {  while (move) { const Bit to = PopBit(move); evaluations.push_back(Callback_Move::template Bishopmove<status, depth>(brd, pos, to)); } }
                }
            }
            Bitloop(bish_nopin) {
                ...
                if constexpr (justcount) { movecnt += Bitcount(move); }
                    ...
                    while (move) { const Bit to = PopBit(move); evaluations.push_back(Callback_Move::template Bishopmove<status, depth>(brd, 1ull << sq, to)); }
                }
            }
        }
        ...
        //Rookmoves
        {
            ...
            Bitloop(rook_pinned) {
                ...
                if constexpr (justcount) { movecnt += Bitcount(move); }
                    ...
                    map pos = 1ull << sq;
                    if (pos & queens) { while (move) { const Bit to = PopBit(move); evaluations.push_back(Callback_Move::template Queenmove<status, depth>(brd, pos, to)); } }
                    else { while (move) { const Bit to = PopBit(move); evaluations.push_back(Callback_Move::template Rookmove<status, depth>(brd, pos, to)); } }
                }
            }
            Bitloop(rook_nopin) {
                ...
                if constexpr (justcount) { movecnt += Bitcount(move); }
                    ...
                    while (move) { const Bit to = PopBit(move); evaluations.push_back(Callback_Move::template Rookmove<status, depth>(brd, 1ull << sq, to)); }
                }
            }
        }
        //Calculate Enemy Queen
        {
            ...
            Bitloop(queens) {
                ...
                if constexpr (justcount) {
                    ...
                    while (move) { const Bit to = PopBit(move); evaluations.push_back(Callback_Move::template Queenmove<status, depth>(brd, 1ull << sq, to)); }
                }
            }
        }
...

        {
            ...
            if ((Lpawns | Rpawns | Fpawns) & Pawns_LastRank<white>())
            {
                ...
                if constexpr (justcount) {
                    ...
                    while (Promote_Left) { 
                        const Bit pos = PopBit(Promote_Left);     eval = Callback_Move::template Pawnpromote<status, depth>(brd, pos, Pawn_AttackLeft<white>(pos), alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
                    ...
                    while (Promote_Right)   { 
                        const Bit pos = PopBit(Promote_Right);    eval = Callback_Move::template Pawnpromote<status, depth>(brd, pos, Pawn_AttackRight<white>(pos), alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
                    ...
                    while (Promote_Move)    { 
                        const Bit pos = PopBit(Promote_Move);     eval = Callback_Move::template Pawnpromote<status, depth>(brd, pos, Pawn_Forward<white>(pos), alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
                    ...
                    while (NoPromote_Left)  { 
                        const Bit pos = PopBit(NoPromote_Left);   eval = Callback_Move::template Pawnatk<status, depth>(brd, pos, Pawn_AttackLeft<white>(pos), alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
                    ...
                    while (NoPromote_Right) { 
                        const Bit pos = PopBit(NoPromote_Right);  eval = Callback_Move::template Pawnatk<status, depth>(brd, pos, Pawn_AttackRight<white>(pos), alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
                    ...
                    while (NoPromote_Move)  {
                        const Bit pos = PopBit(NoPromote_Move);   eval = Callback_Move::template Pawnmove<status, depth>(brd, pos, Pawn_Forward<white>(pos), alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
                    ...
                    while (Ppawns)          { 
                        const Bit pos = PopBit(Ppawns);           eval = Callback_Move::template Pawnpush<status, depth>(brd, pos, Pawn_Forward2<white>(pos), alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
                ...
                if constexpr (justcount) {
                    ...
                    while (Lpawns) { 
                        const Bit pos = PopBit(Lpawns); eval = Callback_Move::template Pawnatk<status, depth>(brd, pos,  Pawn_AttackLeft<white>(pos), alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
                    ...
                    while (Rpawns) { 
                        const Bit pos = PopBit(Rpawns); eval = Callback_Move::template Pawnatk<status, depth>(brd, pos,  Pawn_AttackRight<white>(pos), alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
                    ...
                    while (Fpawns) { 
                        const Bit pos = PopBit(Fpawns); eval = Callback_Move::template Pawnmove<status, depth>(brd, pos, Pawn_Forward<white>(pos), alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
        ...
        //Knightmoves
        {
            ...
            Bitloop(knights) {
                ...
                if constexpr (justcount) {
                    ...
                    while (move) { 
                        const Bit to = PopBit(move); eval = Callback_Move::template Knightmove<status, depth>(brd, 1ull << sq, to, alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
        ...
        //Bishopmoves
        {
            ...
            Bitloop(bish_pinned) {
                ...
                if constexpr (justcount) { movecnt += Bitcount(move); }
                    ...
                    map pos = 1ull << sq;
                    if (pos & queens) { while (move) { 
                        const Bit to = PopBit(move); eval = Callback_Move::template Queenmove<status, depth>(brd, pos, to, alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
                        ...
                        const Bit to = PopBit(move); eval = Callback_Move::template Bishopmove<status, depth>(brd, pos, to, alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
            ...
            Bitloop(bish_nopin) {
                ...
                if constexpr (justcount) { movecnt += Bitcount(move); }
                    ...
                    while (move) { 
                        const Bit to = PopBit(move); eval = Callback_Move::template Bishopmove<status, depth>(brd, 1ull << sq, to, alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
        ...
        //Rookmoves
        {
            ...
            Bitloop(rook_pinned) {
                ...
                if constexpr (justcount) { movecnt += Bitcount(move); }
                    ...
                    if (pos & queens) { while (move) { 
                        ...
                        const Bit to = PopBit(move); eval = Callback_Move::template Rookmove<status, depth>(brd, pos, to, alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
            ...
            Bitloop(rook_nopin) {
                ...
                if constexpr (justcount) { movecnt += Bitcount(move); }
                    ...
                    while (move) { 
                        const Bit to = PopBit(move); eval = Callback_Move::template Rookmove<status, depth>(brd, 1ull << sq, to, alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
        ...
        //Calculate Enemy Queen
        {
            ...
            Bitloop(queens) {
                ...
                if constexpr (justcount) {
                    ...
                    while (move) { 
                        const Bit to = PopBit(move); eval = Callback_Move::template Queenmove<status, depth>(brd, 1ull << sq, to, alpha, beta);
                        value = std::max(value, eval);
                        alpha = std::max(alpha, value);
                        if (alpha >= beta)
                        {
